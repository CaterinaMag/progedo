...
def index(request):
    #object_list = Post.objects.all().order_by('-date') # all sta per 'dammi tutti i post che hai'
    object_list = Post.objects.filter(is_published=True).order_by('-date')
...
def poesie(request):
    return render(request, "blog/poesie.html", {})
